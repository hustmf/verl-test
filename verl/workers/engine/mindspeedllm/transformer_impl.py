...
from .utils import add_config, set_random_seed
from .utils import get_base_mcore_config_from_engine_config,\
...
class MindSpeedLLMEngine(BaseEngine):
    def __init__(
        self,
        ...
        self._parse_args_from_config()
        self._register_megatron_utils()
    ...
    def _parse_args_from_config(self):

        #parse args to apply llm adapter
        print("Enter arg parse")
        origin_sys_argv = sys.argv
        sys.argv = [sys.argv[0]]
        model_config = get_base_mcore_config_from_model_config(self.model_config)
        optimizer_config = get_base_mcore_config_from_optim_config(self.optimizer_config)
        engine_config = get_base_mcore_config_from_engine_config(self.engine_config)
        add_config(model_config)
        add_config(optimizer_config)
        add_config(engine_config)
        sys.argv.append("--spec")
        sys.argv.append("mindspeed_llm.tasks.models.spec.qwen3_spec")
        sys.argv.append("layer_spec")
        print("start patch llm")
    ...
    def _register_megatron_utils(self):
        ...
        self.get_megatron_module_device=get_megatron_module_device
        self.load_megatron_model_to_gpu=load_megatron_model_to_gpu
        self.load_megatron_optimizer=load_megatron_optimizer
        self.offload_megatron_model_to_cpu=offload_megatron_model_to_cpu
        self.offload_megatron_optimizer=offload_megatron_optimizer
        self.register_megatron_training_hooks=register_megatron_training_hooks
    ...
    #     return module
    def _build_megatron_module(self):
        from megatron.training.training import setup_model_and_optimizer, get_model
        from megatron.core.enums import ModelType
        from .utils import gpt_model_provider
        # For forward_only, we don't need optimizer, lr_scheduler, checkpoint_mananager
        if self.engine_config.forward_only:
            self.module = get_model(gpt_model_provider, ModelType.encoder_or_decoder, wrap_with_ddp=False)
            return
        #breakpoint()
        self.module = get_model(gpt_model_provider, ModelType.encoder_or_decoder, wrap_with_ddp=True)
        # self.module, self.optimizer, self.lr_scheduler = setup_model_and_optimizer(
        #     gpt_model_provider, ModelType.encoder_or_decoder)    
        print(f"Load from{self.model_config.local_path}")
        if self.vanilla_bridge:
            self.bridge.load_weights(self.module, self.model_config.local_path) 
    ...
    def _build_optimizer(self):
        ...
        self.register_megatron_training_hooks(self.module, optimizer)
        return optimizer
    ...
    def is_mp_src_rank_with_outputs(self):
        return (
            self.mpu.get_tensor_model_parallel_rank() == 0
            and self.mpu.get_pipeline_model_parallel_rank() == self.mpu.get_pipeline_model_parallel_world_size() - 1
            and self.mpu.get_context_parallel_rank() == 0
        )
    ...
    def to(self, device: str, model: bool = True, optimizer: bool = True, grad: bool = True):
        ...
        if device == device_name:
            if model:
                self.load_megatron_model_to_gpu(self.module, load_grad=grad)
            if optimizer and self.optimizer is not None:
                self.load_megatron_optimizer(self.optimizer)
        elif device == "cpu":
            if model:
                self.offload_megatron_model_to_cpu(self.module)
            if optimizer and self.optimizer is not None:
                self.offload_megatron_optimizer(self.optimizer)
        else:
    ...
    def get_data_parallel_group(self):
        return self.mpu.get_data_parallel_group()
    ...
    def save_checkpoint(
        ...
        **kwargs,
        ...
        if self._is_offload_param or origin_module_device == "cpu":
            self.load_megatron_model_to_gpu(self.module, load_grad=True)
        # self.checkpoint_mananager.save_checkpoint(
        #     local_path=local_path, hdfs_path=hdfs_path, global_step=global_step, max_ckpt_to_keep=max_ckpt_to_keep
        # )
        from verl.utils.megatron_utils import get_hf_model_checkpoint_path
        self.bridge.save_weights(self.module, get_hf_model_checkpoint_path(local_path))
        print("success save bridge weight")
    ...
    def load_checkpoint(
        ...
        """
        if self._is_offload_param:
            self.load_megatron_model_to_gpu(self.module)
    ...
    def forward_backward_batch(self, data: TensorDict, loss_function: Callable, forward_only=False) -> Any:
        ...
        vpp_size = self.mpu.get_virtual_pipeline_model_parallel_world_size()
        ...
        if self.mpu.is_pipeline_last_stage(ignore_virtual=True):
    ...
    def get_per_tensor_param(self, base_sync_done=False, **kwargs):
        self.load_megatron_model_to_gpu(self.module, load_grad=False)
        peft_config = None
...
class EngineEvalModeCtx(BaseEngineCtx):
    def __init__(self, engine: MindSpeedLLMEngine, **kwargs):
        super().__init__(engine=engine, mode="eval", **kwargs)

    def __enter__(self):
        assert isinstance(self.engine, MindSpeedLLMEngine)
        super().__enter__()
...
class MindSpeedLLMEngineWithLMHead(MindSpeedLLMEngine):
