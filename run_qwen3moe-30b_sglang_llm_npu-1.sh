...
export engine_backend="mindspeedllm"
# Necessary env
...
ray start --head --port 6766 --dashboard-host="0.0.0.0"  --node-ip-address=172.27.1.109 --dashboard-port=4919 --resources='{"NPU": '$NPUS_PER_NODE'}'
# Model Weights Paths
MODEL_PATH=/mnt/share/m00876805/ckpt/Qwen3-30B-MoE
MCORE_MODEL_PATH=/mnt/share/m00876805/ckpt/Qwen3-30B-MoE-megatron
RAY_DATA_HOME=${RAY_DATA_HOME:-"${HOME}/verl"}
CKPTS_DIR=/mnt/share/m00876805/ckpt/Qwen3-30B-MoE-0203-1

# File System Paths
TRAIN_FILE=/mnt/share/m00876805/data/dapo-math-17k.parquet
TEST_FILE=/mnt/share/m00876805/data/dapo-math-17k.parquet
# TRAIN_FILE=/mnt/share/m00876805/data/gsm8k/train.parquet
# TEST_FILE=/mnt/share/m00876805/data/gsm8k/test.parquet
...
ACTOR_CONFIG=(
    ...
    actor_rollout_ref.actor.mindspeedllm.expert_model_parallel_size=${train_ep}
    actor_rollout_ref.actor.mindspeedllm.expert_tensor_parallel_size=${train_etp}
    # Memory Optimization
    ...
    actor_rollout_ref.actor.mindspeedllm.dist_checkpointing_path=${MCORE_MODEL_PATH}
    actor_rollout_ref.actor.mindspeedllm.use_dist_checkpointing=True
    actor_rollout_ref.actor.mindspeedllm.use_mbridge=True
    # Transformer Architecture Optimizations
    +actor_rollout_ref.actor.mindspeedllm.override_transformer_config.use_flash_attn=True
    +actor_rollout_ref.actor.mindspeedllm.override_transformer_config.recompute_method=uniform
    +actor_rollout_ref.actor.mindspeedllm.override_transformer_config.recompute_granularity=full
    +actor_rollout_ref.actor.mindspeedllm.override_transformer_config.recompute_num_layers=1
    actor_rollout_ref.actor.mindspeedllm.engine_kwargs.seq_length=10240
    ...
    +actor_rollout_ref.actor.mindspeedllm.engine_kwargs.recompute_num_layers=4
...
REF_CONFIG=(
    ...
    actor_rollout_ref.ref.mindspeedllm.expert_model_parallel_size=${train_ep}
    actor_rollout_ref.ref.mindspeedllm.expert_tensor_parallel_size=${train_etp}
    # Memory Optimization
    actor_rollout_ref.ref.mindspeedllm.param_offload=${all_offload}
    # Model Weights Management
    actor_rollout_ref.ref.mindspeedllm.dist_checkpointing_path=${MCORE_MODEL_PATH}
    actor_rollout_ref.ref.mindspeedllm.use_dist_checkpointing=True
...
python3 -m verl.trainer.main_ppo \
    --config-path=config \
    --config-name='ppo_megatron_trainer_llm.yaml' \
    "${DATA_CONFIG[@]}" \
    "${MODEL_CONFIG[@]}" \
    "${ACTOR_CONFIG[@]}" \
    "${REF_CONFIG[@]}" \
